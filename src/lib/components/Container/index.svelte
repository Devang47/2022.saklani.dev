<script lang="ts">
	export let color: 'purple' | 'red' = 'purple';
</script>
...
<style lang="postcss">
	section {
		...
		@apply w-full md:w-10/12 lg:w-[80%] lg:max-w-6xl mx-auto;
	...
	@keyframes move {
		...
		38% {
			transform: scale(0.8, 1) translate(30%, 55%) rotate(160deg);
		}
		40% {
			transform: scale(0.8, 1) translate(30%, 55%) rotate(160deg);
		}
		78% {
			transform: scale(1.2) translate(-20%, 30%) rotate(-20deg);
		}
		80% {
			transform: scale(1.2) translate(-20%, 30%) rotate(-20deg);
		}
